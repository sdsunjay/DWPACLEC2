...
void INThandler(int sig)
{
	...
	signal(sig, SIG_IGN);
	snprintf(message,128,"You hit Ctrl-C.\n");
	write(1,message,128);
...
int main(int argc, char** argv)
{
	...
	printf("Testing connection to database at %s\n",hostName);
	...
	if(db_flag)
	{
		...
		for (i=0; i<gpu_num; ++i)
		{
			...
			{
				printf("GPU %d: Connecting to DB: ",i);
				printf("fail\n");
				exit(0);
			}
		}
	...
	if (flag)
	{
		fprintf(stderr,"can't create thread for gpu: %s\n", strerror(flag));
